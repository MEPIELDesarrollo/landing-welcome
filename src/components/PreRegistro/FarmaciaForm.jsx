...
export function NavButtons({ step, totalSteps, onBack, onNext, onSubmit, disabled}) {
  const btn =
    ...
    <div className="relative w-full h-11" style={{ transform: 'translateY(-50%)' }}>
      ...
      {step === 1 && (
        <div className="flex justify-center">
          <button onClick={onNext} type="button" className={btn} disabled={disabled}>Continuar</button>
        </div>
      )}
      ...
      {step === 2 && (
        <>
          <div className="absolute left-20 top-0">
            <button onClick={onBack} type="button" className={btn} disabled={disabled}>Atrás</button>
          </div>
          <div className="absolute right-20 top-0">
            <button onClick={onSubmit} type="button" className={btn} disabled={disabled}>{disabled ? 'Enviando...' : 'Enviar'}</button>
          </div>
        </>
      )}
  ...
  return (
    <>
      ...
      <div className="flex flex-col gap-4 pb-9">
        <TextInput label="Nombre del negocio" id="nombreNegocio" required value={data.nombreNegocio} onChange={set('nombreNegocio')} />
        <SelectInput
          label="Uso de CFDI"
          id="cfdi"
          required
...
export default function FarmaciaForm({ onStepChange }) {
  ...
  const [step2Data, setStep2Data] = useState({
    nombreNegocio: '', cfdi: '', ine: null, compDom: null, constancia: null,
  });
  ...
  const handleFormSubmit = async (e) => {
    ...
    if (step === 2) {
      ...
      try {
        ...
        const datosUnificados = { ...step1Data, cfdi: step2Data.cfdi, nombreNegocio: step2Data.nombreNegocio, tipoForm: "FARMACIA" };
